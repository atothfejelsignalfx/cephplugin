...
class CollectdCephStorage(object):
    ...
    def _formatDimsForSignalFx(self, dims):
        formatted = ",".join(["{0}={1}".format(d, dims[d]) for d in dims])
        return "[{0}]".format(formatted) if formatted != "" else ""

    def dispatch_value(self, plugin_instance, type_instance, metricvalue, interval):
        #build SignalFx metric
        ArrayForMetric = []
        dimensions = {}
        ArrayForMetric.append("{0}{1}".format(plugin_instance, type_instance))
        metric = collectd.Values(type="gauge")
        metric.plugin = 'collectd-ceph-storage'
        #make interval a dimension
        dimensions[interval] = interval
        #metric.interval = interval
        #metric.plugin_instance = plugin_instance
        metric.type_instance = "{0}{1}".format(ArrayForMetric, self._formatDimsForSignalFx(dimensions))
	#collectd.info(metric.type_instance)
...
#        collectd.register_read(read_callback, interval=self.ceph_rados_bench_interval)
#        return True
	

collectd_ceph_storage = CollectdCephStorage()
collectd.register_config(collectd_ceph_storage.configure_callback)
#collectd.register_init(collectd_ceph_storage.init_callback)
